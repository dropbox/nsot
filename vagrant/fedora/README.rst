...
Install NSoT on it's own Linux environment: A Fedora virtual machine, with
...
the server from one file. The ``Vagrantfile`` for Fedora can be found in the
same directory as this README document. If you would like to install NSoT on
CentOS, Debian, SuSe, or Ubuntu via Vagrantfile, please `download the file for
that architecture.
<https://github.com/dropbox/nsot/tree/master/vagrant>`_
